    ...
    .mutation(async ({ input, ctx }): Promise<EscrowReleaseResponse> => {
      ...
      try {
        ...
        if (populatedStore?.storeEmail) {
          try {
            // Render escrow release email template
            const html = await renderTemplate(
              React.createElement(EscrowReleaseEmail, {
                storeName: populatedStore.name || "Store Owner",
                orderId: (
                  orderWithSubOrder._id as mongoose.Types.ObjectId
                ).toString(),
                storeId: subOrder.storeId.toString(),
                subOrderId: subOrder._id?.toString() || "",
                amountReleased: settlementDetails.releaseAmount,
            ...
            const notification = NotificationFactory.create("email", {
              recipient: populatedStore.storeEmail,
