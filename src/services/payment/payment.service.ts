 ...
 *
 * The business steps that must happen for EVERY provider â€” cart validation,
 * pending-order creation (which records the chosen gateway on the order),
 * building the neutral initiation payload â€” live here, once. The gateway
...
export class PaymentService {
   ...
   * updated to the next candidate and initiation is retried â€” the customer
  ...
  static async initializePayment({
    ...
    // â”€â”€ Step 1: Validate cart â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    // â”€â”€ Step 2: Create the pending order, recording WHICH gateway will
    // collect the payment. The order record is the single source of truth
    // for the provider from this point on â€” the status page and webhook
    ...
    // â”€â”€ Step 3: Try each candidate in order â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
      ...
      try {
        ...
        console.error(
          `[PaymentService] Payment initiation failed on ${gateway}` +
            (i < candidates.length - 1 ? " â€” failing over" : ""),
          error,
        );
      }
    }
    ...
    // rejects ANY existing order for that key â€” Failed and Cancelled included.
   ...
   * braces rather than a live race â€” no payment link reached the customer â€” but
      ...
      if (result.deletedCount === 0) {
        console.warn(
          `[PaymentService] Order ${orderId} was not discarded after failed ` +
            `initiation â€” it is no longer Pending. Left for manual review.`,
        );
      }
    } catch (error) {
