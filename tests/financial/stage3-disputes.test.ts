...
async function createDeliveredOrderDocument(order: SeededPaidOrder) {
  ...
  await Order.create({
    ...
    idempotencyKey: `DISPUTE-TEST-${order.orderId.toString()}`,
  });
}
...
describe("Stage 3 — disputes", () => {
  beforeAll(async () => {
    await startTestDb();
    await ensureCollections(); // ← creates every collection once, before any txn
  });
  ...
  it("real service: open → reject keeps the ledger balanced", async () => {
    ...
    await createDeliveredOrderDocument(order);
    ...
    const walletAfterOpen = await getVendorWalletByVendorId(
      vendorId.toString(),
    );
    expect(walletAfterOpen!.balances).toMatchObject({
      available: 0,
      disputed: order.suborders[0]!.settleAmount,
      pending: 0,
      total: order.suborders[0]!.settleAmount,
    });
    ...
    const result = await DisputeService.rejectDispute({
      ...
      resolutionNotes: "Vendor was not at fault.",
    });

    expect(result.dispute.outcome).toBe(DisputeOutcome.REJECTED);
    expect(result.releasedAmount).toBe(order.suborders[0]!.settleAmount);

    const walletAfterReject = await getVendorWalletByVendorId(
      vendorId.toString(),
    );
    expect(walletAfterReject!.balances).toMatchObject({
      available: order.suborders[0]!.settleAmount,
      disputed: 0,
      pending: 0,
      total: order.suborders[0]!.settleAmount,
    });
  ...
  it("real service: open → uphold with available cover preserves ledger integrity", async () => {
    const vendorId = new mongoose.Types.ObjectId();
    await seedVendorWallet(vendorId);

    const extra = await seedSettledSuborder({
      grossAmount: 300_000,
      vendorId,
    });
    ...
    const result = await DisputeService.upholdDispute({
      disputeId: dispute._id.toString(),
      resolutionNotes: "Customer receives full refund.",
      resolvedBy: DisputeResolvedBy.PLATFORM_TEAM,
    });
    ...
    expect(wallet!.balances.available).toBe(
      extra.suborders[0]!.settleAmount - result.penaltyAmount,
    );
    expect(wallet!.balances.disputed).toBe(0);
    ...
    expect(platform!.balances.penalties).toBe(result.penaltyAmount);

    const solvency = await expectSystemConsistent([vendorId.toString()]);
    expect(solvency.liabilities.customerRefundPayable).toBe(order.totalAmount);
  });

  it("real service: open → uphold with system resolution creates full refund and zero penalty", async () => {
    const vendorId = new mongoose.Types.ObjectId();
    await seedVendorWallet(vendorId);
    ...
    const result = await DisputeService.upholdDispute({
      disputeId: dispute._id.toString(),
      resolutionNotes: "Auto-resolved after deadline.",
